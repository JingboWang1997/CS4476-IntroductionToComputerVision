...
def detectAllCircles(im, minRadius, maxRadius):
    ...
    # higher = 0.8
    # maximum = accumulatorArrays.max()
    # lowerValue = lower * maximum
    # higherValue = higher * maximum
    # accumulatorArrays[np.where(accumulatorArrays < lowerValue)] += maximum
    # locations = np.where(accumulatorArrays < higherValue)
    # centers = []
    # for i in xrange(len(locations[0])):
    #     centers.append((locations[1][i], locations[0][i], locations[2][i] + minRadius))
    # return centers